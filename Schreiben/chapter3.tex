...
\end{equation*}  
wobei für \(i > r\) gilt, dass \(\lambda_{i} = 0\).
...
\begin{repitition}\label{rep:alma}\leavevmode
    ...
    \end{itemize}
\end{repitition}
Wir wollen nun zeigen, dass sich die Konditionszahl von \(\Sigma\) im Vergleich zu \(X\) erhöht und \(\Sigma\) somit anfälliger für Fehler bei numerischen Berechnungen, wie beispielsweise der Bestimmung von Eigenwerten ist.  
Dafür benötigen wir die Singulärwerte von \(\Sigma\), welche als Wurzeln der positiven Eigenwerte von \(\Sigma^{T}\Sigma = \Sigma^{2}\) gegeben sind.
Betrachte also für beliebigen Eigenvektor \(v\) von \(\Sigma\) mit Eigenwert \(\lambda\)
\begin{equation*}
    \Sigma^{2}v = \Sigma \lambda v = \Sigma v \lambda = \lambda^{2} v.
\end{equation*}   
Die Singulärwerte von \(\Sigma\) sind damit gegeben durch die Eigenwerte von \(\Sigma\), womit  
\begin{equation*}
    \kappa(\Sigma) = \frac{\sigma_{1}^{2}/n}{\sigma_{r}^{2}/n}={\left(\frac{\sigma_{1}}{\sigma_{r}}\right)}^{2} = {\kappa(X)}^{2}.
\end{equation*}  
Die Konditionszahl wird folglich quadriert, was insbesondere bei schlecht konditionierten Matrizen zu deutlichen Ungenauigkeiten führen kann.

Um diese abstrakte Folgerung zu verdeutlichen, veranschaulichen wir dies an einem Beispiel.
Sei 
\begin{equation*}
    X = 
    \begin{bmatrix}
    ...
    \end{bmatrix},
\end{equation*}
auch als \emph{Läuchli-Matrix} bekannt.
Die quadrierten Singulärwerte von \(X\) sind bekannt als \(3 + \varepsilon^{2},\ \varepsilon^{2}\) und erneut \(\varepsilon^{2}\).  
Nun berechnen wir die quadrierten Singulärwerte mithilfe von \texttt{Python} auf zwei verschiedene Weisen.
Zum einen mithilfe der SVD von \(X\) und zum anderen durch direkte Berechnung der Eigenwerte von \(X^{T}X\). 
Dafür verwenden wir folgenden Code:
\begin{minted}{python}
epsilons = [1e-3, 1e-10, 1e-20]

for epsilon in epsilons:
    X = laeuchli_matrix(epsilon)
...
-
\end{outputcode}
Wir können beobachten, dass zwar für \(\varepsilon = 10^{-3}\) beide Vorgehensweise das gleiche Ergebnis liefern, für \(\varepsilon=10^{-10}\) und \(\varepsilon = 10^{-20}\) allerdings die Eigenwertberechnung nicht mehr exakt arbeitet. 